...
function OfferCards({ offer }: { offer: OfferCard }) {
  return (
    <Link
      href={offer.href}
      className={`group relative flex flex-col justify-between h-[200px] md:h-[250px] overflow-hidden rounded-[10px] ${offer.bgColor}
  ...
  lg:hover:-translate-y-3`}
    >
      ...
      {/* Content */}
      <div className="relative z-20 flex flex-col items-center text-center px-2 pt-4 lg:pt-6">
        {/* Badge */}
        ...
        {/* Title */}
        <h2 className="mt-3 text-[14px] md:text-[15px] font-bold max-w-[240px] font-semibold leading-[0.95] tracking-tight text-black line-clamp-2">
          {offer.title}
        </h2>

        {/* Subtitle */}
        <p className="mt-2 text-[7px] md:text-[9px] uppercase tracking-[0.35em] text-black/45">
          {offer.subtitle}
        </p>
      </div>

      {/* Product Image */}
      <div className="relative z-10 flex items-end justify-center pb-3 px-4">
        <Image
          src={offer.image}
          alt={offer.title}
          width={200}
          height={200}
          className="w-auto h-[90px] md:h-[120px] max-w-[130px] md:max-w-[175px] object-contain transition-all duration-700 ease-out group-hover:scale-105"
        />
      </div>
      ...
      {/* Hover Shine */}
      <div
        className="absolute inset-0 -translate-x-full skew-x-12 bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-1000 group-hover:translate-x-[250%]" />
    </Link>
  );
}
